...
let isResumingProcess = false;     // v167: レジューム処理中フラグ（上書き防止用）
...
function savePlaybackState() {
    try {
        // v167: レジューム処理中は保存しない（0秒で上書きされるのを防ぐ）
        if (isResumingProcess) return;
...
const syncUI = () => {
    try {
        ...
        if (audio) {
            ...
            const progress = (audio.duration > 0 && isFinite(audio.duration)) ? (audio.currentTime / audio.duration) * 100 : 0;
...
async function playWithAmplitude(index, startTime = 0, shouldPlay = true) {
    ...
    try {
        ...
        if (targetId) {
            ...
            const startMs = performance.now();
            try {
                // v168: カバーアート用の取得はタイムアウトを短くし (10秒)、失敗しても再生自体は続行する
                blob = await Promise.race([fetchFile(), downloadTimeout(10000)]).finally(() => {
                    MetadataQueue.resume();
                });
            } catch (err) {
                if (err.name === 'AbortError') throw err;
                console.warn(`[Download] CoverArt fetch failed/timeout (FileID: ${targetId}):`, err);
                // 失敗しても再生プロセスは止めない
                blob = null;
            }
...
function startPlayback(index, url, cover, gen, startTime = 0, shouldPlay = true) {
    ...
    if (audio && (url.startsWith('blob:') || url.includes('/proxy-stream'))) {
        ...
        if (shouldPlay) {
            // v167: 再生前に時間をセットできるか試みる（メタデータがあれば即座に）
            if (startTime > 0 && audio.readyState >= 1) {
                console.log(`[Resume] Setting startTime immediately: ${startTime}s`);
            ...
            audio.play().then(() => {
                ...
                if (e.name === 'NotAllowedError') {
                    ...
                    // v167: ブロックされた場合でも position は維持されるはずだが
                    // UIの状態を「再生しようとしたが止まっている」状態として見せる
                    updateStatus("Autoplay blocked. Click Play to resume.");
                } else {
    ...
    if (audio) {
        ...
        // v95/167/168: startTime が指定されている場合はシーク
        if (startTime > 0) {
            const applyStartTime = () => {
                if (isFinite(startTime) && audio.duration > 0 && isFinite(audio.duration)) {
                    console.log(`[Resume] Setting startTime via event (${startTime}s, readyState=${audio.readyState}, duration=${audio.duration})`);
                    // duration を超えていないかチェック
                    const target = Math.min(startTime, audio.duration - 0.1);
                    audio.currentTime = Math.max(0, target);
                }
                audio.removeEventListener('canplay', applyStartTime);
                audio.removeEventListener('loadedmetadata', applyStartTime);
                audio.removeEventListener('loadeddata', applyStartTime);
                // 少し遅らせてフラグを解除（保存処理の再開）
                setTimeout(() => { isResumingProcess = false; }, 2000);
            };
            ...
            if (audio.readyState >= 1) {
                ...
                audio.addEventListener('loadeddata', applyStartTime);
                // 万が一イベントが来ない場合のためにタイムアウトで解除
                setTimeout(() => { 
                    if (isResumingProcess) {
                        console.warn("[Resume] applyStartTime timeout. Force clearing flag.");
