...
(function () {
    ...
    var JUNO_ARTIST_SERCH_TRAILER = "&order_by=-release_date";
    ...
    function trxsrc_run() {
        ...
        if (!$("#download_txtfile")[0] && Object.keys(my_mp3_tracks).length != 0) {
                ...
                if (num_elm) {
                    ...
                    for (let i = 0; i < artist_elms.length; i++) {
                        ...
                        if (a.attr("href").startsWith(TRAXSOURCE_URL) || !a.attr("href").startsWith("http")) {
                            ...
                            let artist_name = artist_name_array.join('+');
                            let title_name_array = title.split(' ');
                            title_name_array.forEach(function (elm, idx) {
                                title_name_array[idx] = encodeURIComponent(elm);
                            });
                            let title_name = title_name_array.join('+');
                            let juno_search_link = JUNO_ARTIST_SERCH_HEADER + artist_name + "+" + title_name + JUNO_ARTIST_SERCH_TRAILER;
                            //a.attr("href", JUNO_ARTIST_SERCH_HEADER+artist_name);
                            a.replaceWith("<a href=\"" + juno_search_link + "\">" + a.text() + "</a>");
                            artist_elms[i].href = juno_search_link;
                            console.log("Traxsource2Juno: " + a.text() + " -> " + a.attr("href"));
                        }
                    }
            ...
            if (the_chart.chart.length) {
                ...
                let new_h1_title = "<a href='https://www.beatport.com/ja/search/releases?q=" +
                    encodeURIComponent(the_chart.chart_title.cleansing()) + "'>" + the_chart.chart_title + "</a>"
                $("h1.title").html(new_h1_title);
                ...
                $("h1.title").append(" - ");
                $("h1.title").append($("<img src=\"https://www.beatport.com/images/beatport-logo-icon.svg\" " +
